...

        {
          !isExternalLinks? <ul className='list-none hidden sm:flex flex-row gap-10'>
            ...
            <li
              key={nav.id}
              className={`${
                "Registeration"===nav.title? "text-white":"text-secondary"
              } hover:text-white text-[18px] font-medium cursor-pointer`}
              onClick={() => setActive(nav.title)}
            >
          ...
          >
            {
              !isExternalLinks? <ul className='list-none flex justify-end items-start flex-1 flex-col gap-4'>
                ...
                {navLinks.map( ( nav ) => (
                  <li
                    key={nav.id}
                    className={`font-poppins text-center font-medium cursor-pointer text-[16px] ${"Registeration"===nav.title? "text-white":"text-secondary"
                      }`}
                    onClick={() => {
                      setToggle( !toggle );
                      setActive( nav.title );
